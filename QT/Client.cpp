...
*/
/*int Client::qt_receive(char *ID,char *msg)
{
	char *oriMsg, *revID;
	int err = recvMessage(oriMsg);
	if (oriMsg[1] == '1')
	{
		parseMessage(oriMsg, ID, msg, revID);
		return 1;
	}
	else
	{
		int tag = parseMessage(oriMsg, msg);
		if (tag != 5)
			ID = msg;
		return 4;
	}
...
int Client::qt_firOnlineID(char* ID)
{
	char *oriMsg;
	char *t;
	recvMessage(oriMsg);
	...
	{
		HANDLE listenThread = CreateThread(NULL, 0, Listen, (LPVOID)this, 0, &listenThreadId);
		if (listenThreadId == NULL)
		{
			cout << "启动监听线程失败";
		}
		return 1;
